...
def create_docs(user_pdf_list):
    ...
    # creating a dataframe with neccessary column names
    ...
    for filename in user_pdf_list:
        ...
        #capturing just the required data and excluding any unwanted text from the LLM response
        pattern = r'{(.+)}'
        match = re.search(pattern, llm_extracted_data, re.DOTALL)

        if match:
            extracted_text = match.group(1)
            # converting the extracted ytext to a dictionary
